...
def write_iceberg_table(
    ...
    logger.debug(
        f"pyiceberg: {write_disposition} to table {table.name()} at"
        f" location {table.location()} took"
...
def merge_iceberg_table(
    ...
    """Merges Arrow data into on-disk Iceberg table via ``txn.upsert``."""
    start_ts = time.monotonic()
    ...
    if strategy in ("upsert", "insert-only"):
        ...
        # pyiceberg 0.10's upsert scans using the current snapshot's schema_id.
        # After schema evolution the snapshot still references the old schema,
        # so the scan returns fewer columns than expected. Force a new snapshot
        # with the updated schema_id by appending an empty table.
        snapshot = table.current_snapshot()
        if snapshot and snapshot.schema_id != table.metadata.current_schema_id:
            from pyiceberg.io.pyarrow import schema_to_pyarrow

            table.append(schema_to_pyarrow(table.metadata.schema()).empty_table())
    ...
    logger.debug(
        f"pyiceberg: merge ({strategy}) to table {table.name()} at"
        f" location {table.location()} took"
        f" {(time.monotonic() - start_ts)} seconds."
    )
